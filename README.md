# Awesome Android Open Source Projects with stars

<h1 align="center">Awesome Android Open-Source Projects</h1><br>

<p align="center">
:eyeglasses: A curated list of awesome android projects by open-source contributors.
</p><br>

<p align="center">
  <a href="#"><img alt="Android OS" src="https://img.shields.io/badge/OS-Android-3DDC84?style=flat-square&logo=android"></a>
  <a href="#"><img alt="Languages-Kotlin" src="https://flat.badgen.net/badge/Language/Kotlin?icon=https://raw.githubusercontent.com/binaryshrey/Awesome-Android-Open-Source-Projects/master/assets/Kotlin_Logo_icon_white.svg&color=f18e33"/></a>
  <a href="#"><img alt="Languages-Java" src="https://img.shields.io/badge/Language-Java-1DA1F2?style=flat-square&logo=java"></a>
  <a href="#"><img alt="OpenSource" src="https://flat.badgen.net/badge/Open-Source/HacktoberFest?icon=https://raw.githubusercontent.com/binaryshrey/Awesome-Android-Open-Source-Projects/master/assets/DO_Logo_icon_white.svg&color=f18e33"/></a>
  <a href="#"><img alt="PRs" src="https://img.shields.io/badge/PRs-Welcome-3DDC84?style=flat-square"></a>
</p>
<p align="center">
  <a href="https://github.com/binaryShrey"><img alt="Github - binaryShrey" src="https://img.shields.io/badge/GitHub-binaryShrey-181717?style=flat-square&logo=github"></a>
  <a href="https://twitter.com/binaryShrey"><img alt="Twitter - binaryShrey" src="https://img.shields.io/badge/Twitter-binaryShrey-1DA1F2?style=flat-square&logo=twitter"></a>
</p>

<br>
<p align="center">
<img width="720px" src="https://raw.githubusercontent.com/binaryshrey/Awesome-Android-Open-Source-Projects/master/assets/AAOSP.webp" alt="aaosp"></img>
</p><br>

<p> Awesome Android Open-Source Projects aims to be the starting point for those who are new to the world of Open-Source and want to contribute for the same. It also serves experienced contributors to participate and contribute in the mentioned list of projects.
</p><br>

## Contents

:shamrock:  [**Categories**](#shamrock--categories)

* :tv: [Android TV](#tv-android-tv)
* :watch: [Android Wear](#watch-android-wear)
* :office: [Architecture](#office-architecture)
* :iphone: [Communication](#iphone-communication)
* :book: [eBook Reader](#book-ebook-reader)
* :movie\_camera: [Entertainment](#movie_camera-entertainment)
* :moneybag: [Finance](#moneybag-finance)
* :soccer: [Games](#soccer-games)
* :weight\_lifting: [Health & Fitness](#weight_lifting-health--fitness)
* :books: [Libraries](#books-libraries)
* :art: [Misc](#art-misc)
* :newspaper: [News](#newspaper-news)
* :rocket: [Productivity](#rocket-productivity)
* :hammer\_and\_wrench: [Tools](#hammer_and_wrench-tools)
* :earth\_africa: [Travel](#earth_africa-travel)
* :gear: [Utilities](#gear-utilities)
* :partly\_sunny: [Weather](#partly_sunny-weather)

:dart:  [**HacktoberFest**](#dart--hacktoberfest)

:memo:  [**Contributing**](#memo--contributing) <br><br>

## :shamrock:  **Categories**

### :tv: Android TV

* [Kodi TV](https://github.com/xbmc/xbmc) ⭐ 21,092 | 🐛 432 | 🌐 C++ | 📅 2026-08-13

  * Kodi is an award-winning free and open source home theater/media center software and entertainment hub for digital media. With its beautiful interface and powerful skinning engine, it's available for Android, BSD, Linux, macOS, iOS, tvOS and Windows.

* [SmartYouTubeTV](https://github.com/yuliskov/SmartYouTubeTV) ⚠️ Archived

  * SmartYouTubeTV: A YouTube client for set-top boxes and Android-based TVs.

* [Tv-samples](https://github.com/android/tv-samples) ⭐ 1,267 | 🐛 64 | 🌐 Kotlin | 📅 2026-07-22

  * This repository consists of multiple samples showing best practices in app development on Android TV.

* [Vineyard](https://github.com/hitherejoe/Vineyard) ⭐ 718 | 🐛 10 | 🌐 Java | 📅 2018-01-04

  * Vineyard is an unofficial Vine client for Android TV.

* [AndroidTVMovieParadise](https://github.com/halilozel1903/AndroidTVMovieParadise) ⭐ 207 | 🐛 0 | 🌐 Java | 📅 2026-07-04

  * Android TV is a version of the Android operating system. It is developed by Google for soundbars, set-top boxes, digital media players, and TVs with native applications.Movie Paradise is an Android TV app. Application is using The Movie Database.

### :watch: Android Wear

* [Wear-os-samples](https://github.com/android/wear-os-samples) ⭐ 1,389 | 🐛 14 | 🌐 Kotlin | 📅 2026-08-12

  * Multiple samples showing best practices in app and watch face development on Wear OS.

* [FORMWatchFace](https://github.com/romannurik/FORMWatchFace) ⚠️ Archived

  * This is a watch face for Android Wear based on the typeface used for the FORM design conference.

* [Twelveish](https://github.com/LayoutXML/Twelveish) ⚠️ Archived

  * Twelveish is a unique Wear OS (Android Wear) Watch Face that displays the approximate time in words (in multiple languages).

* [Wear-notify-for-reddit](https://github.com/emmaguy/wear-notify-for-reddit) ⭐ 49 | 🐛 10 | 🌐 Java | 📅 2016-06-11

  * Get notifications from your favourite subreddits right on your android wearable.

* [Smartalarm](https://github.com/fridgecow/smartalarm) ⭐ 44 | 🐛 10 | 🌐 Java | 📅 2019-08-29

  * Smart Alarm is a privacy-focused sleep tracker with powerful features and works without an internet or phone connection.

* [WearSquare](https://github.com/davidvavra/WearSquare)

  * Unofficial Foursquare® Client for Android Wear.

### :office: Architecture

* [Android-mvp-architecture](https://github.com/MindorksOpenSource/android-mvp-architecture) ⭐ 4,420 | 🐛 40 | 🌐 Java | 📅 2024-04-16

  * This repository contains a detailed sample app that implements MVP architecture using Dagger2, GreenDao, RxJava, FastAndroidNetworking, PlaceHolderView and AndroidDebugDatabase.

* [Archi](https://github.com/ivacf/archi) ⭐ 3,302 | 🐛 12 | 🌐 Java | 📅 2019-10-03

  * This repository showcases and compares different architectural patterns that can be used to build Android apps. The exact same sample app is built three times with different architectures.

* [Android-mvvm-architecture](https://github.com/MindorksOpenSource/android-mvvm-architecture) ⭐ 2,951 | 🐛 40 | 🌐 Java | 📅 2024-04-16

  * This repository contains a detailed sample app that implements MVVM architecture using Dagger2, Room, RxJava, FastAndroidNetworking, PlaceHolderView and AndroidDebugDatabase.

* [Foodium](https://github.com/PatilShreyas/Foodium) ⭐ 2,325 | 🐛 5 | 🌐 Kotlin | 📅 2026-03-20

  * Foodium is a sample food blog Android application built to demonstrate use of Modern Android development tools - (Kotlin, Coroutines, Flow, Dagger 2, Architecture Components, MVVM, Room, Retrofit, Moshi, Material Components).

* [NotyKT](https://github.com/PatilShreyas/NotyKT) ⭐ 1,768 | 🐛 12 | 🌐 Kotlin | 📅 2026-04-23

  * NotyKT is a complete Kotlin-stack (Backend + Android) application built to demonstrate the use of Modern development tools with best practices implementation.

* [Clean-Notes](https://github.com/mitchtabian/Clean-Notes) ⭐ 556 | 🐛 3 | 🌐 Kotlin | 📅 2024-03-13

  * Android example with Clean Architecture by layer.

* [Baking-App-Kotlin](https://github.com/Ezike/Baking-App-Kotlin) ⭐ 457 | 🐛 1 | 🌐 Kotlin | 📅 2021-08-24

  * Android architecture sample with dynamic feature modularisation, clean architecture with MVI (Uni-directional data flow), dagger hilt, DFM Navigation, kotlin coroutines with StateFlow and Exo player.

* [RestApiMVVM](https://github.com/mitchtabian/RestApiMVVM) ⭐ 148 | 🐛 1 | 🌐 Java | 📅 2019-11-18

  * App that interacts with a Rest Api. Architecture is MVVM.

* [Github Visualizer](https://github.com/DheerajKotwani/GithubVisualizer) ⭐ 121 | 🐛 1 | 🌐 Kotlin | 📅 2021-04-21

  * Android Application to track any user activity on Github built using the Github Developers Api. The tools used for implementations - Github Developers Api, Kotlin, Coroutines, MVVM, Glide, Retrofit.

* [JetJoke](https://github.com/swapnil-musale/JetJoke) ⭐ 46 | 🐛 13 | 🌐 Kotlin | 📅 2026-08-11

  * JetJoke is a Joke generator app build using Jetpack Compose & Clean + MVVM Architecture to demonstrate it's use with other modern Android Development libraries and tools.

* [Clean-MVP](https://github.com/carlosmacmar/clean-MVP) ⭐ 21 | 🐛 0 | 🌐 Kotlin | 📅 2019-02-13

  * Kotlin MVP clean architecture for Android using Koin, Room and RxJava2

### :iphone: Communication

* [Telegram](https://github.com/DrKLO/Telegram) ⭐ 29,709 | 🐛 467 | 🌐 Java | 📅 2026-08-02

  * Telegram is a messaging app with a focus on speed and security. It’s superfast, simple and free. This repo contains the official source code for Telegram App for Android.

* [Signal-Android](https://github.com/WhisperSystems/Signal-Android) ⭐ 29,193 | 🐛 476 | 🌐 Kotlin | 📅 2026-08-13

  * Signal is a messaging app for simple private communication with friends.

* [Jetchat](https://github.com/android/compose-samples/tree/master/Jetchat) ⭐ 23,375 | 🐛 7 | 🌐 Kotlin | 📅 2026-08-13

  * Jetchat is a sample chat app built with Jetpack Compose.

* [K-9](https://github.com/k9mail/k-9) ⭐ 13,848 | 🐛 1,060 | 🌐 Kotlin | 📅 2026-08-13

  * K-9 Mail is an open-source email client for Android.

* [Tutanota](https://github.com/tutao/tutanota) ⭐ 7,852 | 🐛 962 | 🌐 TypeScript | 📅 2026-08-13

  * Tutanota is an email client with a strong focus on security and privacy that lets you encrypt emails on all your devices.

* [Qksms](https://github.com/moezbhatti/qksms) ⭐ 4,590 | 🐛 525 | 🌐 Kotlin | 📅 2023-06-02

  * QKSMS is an open source replacement to the stock messaging app on Android. It is currently available on the Google Play Store and on F-Droid

* [FairEmail](https://github.com/M66B/FairEmail/) ⭐ 4,544 | 🐛 3 | 🌐 Java | 📅 2026-08-10

  * An advanced email client for Android

* [Kdeconnect-android](https://github.com/KDE/kdeconnect-android) ⭐ 1,413 | 🐛 0 | 🌐 Kotlin | 📅 2026-08-13

  * Native Android port of the KDE Connect Qt app

* [Proton Mail for Android](https://github.com/ProtonMail/android-mail) ⭐ 964 | 🐛 1 | 🌐 Kotlin | 📅 2026-07-17

  * ProtonMail Android app

* [Jetpack-Compose-WhatsApp-Clone](https://github.com/MindorksOpenSource/Jetpack-Compose-WhatsApp-Clone) ⭐ 667 | 🐛 6 | 🌐 Kotlin | 📅 2024-02-19

  * An example project to demonstrate how to build WhatsApp using Jetpack Compose.

* [forem-Dev](https://github.com/forem/DEV-Android) ⭐ 424 | 🐛 16 | 🌐 Kotlin | 📅 2026-02-12

  * The is in official Dev.to Android App. Dev.to is a community for Developers to engage with other Developers.

* [Tindroid](https://github.com/tinode/tindroid) ⭐ 394 | 🐛 4 | 🌐 Java | 📅 2026-07-12

  * Android instant messenger using Tinode backend. Beta-quality software. Debug APKs are available in releases.

* [Irccloud-android](https://github.com/irccloud/android) ⭐ 309 | 🐛 32 | 🌐 Java | 📅 2026-04-28

  * The official Android app for IRCCloud.com

* [Wire-android](https://github.com/wireapp/wire-android) ⭐ 254 | 🐛 29 | 🌐 Kotlin | 📅 2026-08-13

  * The project in this repository contains the Wire for Android client project.

* [Replify-Messenger](https://github.com/vidit135g/Replify-Messenger) ⭐ 25 | 🐛 1 | 🌐 Kotlin | 📅 2020-07-10

  * Minimal text messenger with a ton of features.

### :book: eBook Reader

* [IReader](https://github.com/IReaderorg/IReader) ⭐ 918 | 🐛 24 | 🌐 Kotlin | 📅 2026-08-06

  * IReader is a next-generation e-book reader, 100% open source, highly customizable, and designed for smooth reading and library management.

* [Bookdash-android-app](https://github.com/bookdash/bookdash-android-app) ⭐ 712 | 🐛 9 | 🌐 Java | 📅 2025-02-12

  * An Android app that lets you download free children's books in different languages from non-profit publisher Book Dash

* [ComicReaderApp\_MVI\_Coroutine\_RxKotlin\_Jetpack](https://github.com/hoc081098/ComicReaderApp_MVI_Coroutine_RxKotlin_Jetpack) ⭐ 292 | 🐛 9 | 🌐 Kotlin | 📅 2026-08-07

  * Comic reader app. Learning MVVM / MVI with RxKotlin, Retrofit, Kotlin Coroutine, Work Manager, Room, Firebase, AndroidX, Clean Architecture.

* [Hummingbird-for-Android](https://github.com/xiprox/Hummingbird-for-Android) ⚠️ Archived

  * A cool, modern client app for the anime tracking website Hummingbird.me

* [AbsolutReader](https://github.com/xvalme/AbsolutReader) ⭐ 15 | 🐛 7 | 🌐 JavaScript | 📅 2022-08-21

  * AbsolutReader is the next generation e-book reader, 100% Open source, highly customizable, and featuring Chaimager.

* [tachiyomi](https://github.com/inorichi/tachiyomi)

  * Free and open source manga reader for Android

### :movie\_camera: Entertainment

* [NewPipe](https://github.com/TeamNewPipe/NewPipe) ⭐ 39,326 | 🐛 1,454 | 🌐 Java | 📅 2026-08-13

  * A libre lightweight streaming front-end for Android.

* [Jetcaster](https://github.com/android/compose-samples/tree/master/Jetcaster) ⭐ 23,375 | 🐛 7 | 🌐 Kotlin | 📅 2026-08-13

  * Jetcaster is a sample podcast app, built with Jetpack Compose. The goal of the sample is to showcase dynamic theming and full featured architecture.

* [AntennaPod](https://github.com/AntennaPod/AntennaPod) ⭐ 8,077 | 🐛 353 | 🌐 Java | 📅 2026-08-02

  * This is the official repository of AntennaPod, the easy-to-use, flexible and open-source podcast manager for Android.

* [Timber](https://github.com/naman14/Timber) ⭐ 7,041 | 🐛 212 | 🌐 Java | 📅 2023-04-28

  * Material Design Music Player

* [Phonograph](https://github.com/kabouzeid/Phonograph) ⚠️ Archived

  * A material designed music player for Android

* [MovieGuide](https://github.com/esoxjem/MovieGuide) ⭐ 2,598 | 🐛 1 | 🌐 Java | 📅 2025-09-23

  * Movie discovery app showcasing MVP, RxJava, Dagger 2 and Clean Architecture

* [Shuttle](https://github.com/timusus/Shuttle) ⚠️ Archived

  * Shuttle is an open source, local music player for Android.

* [MovieHunt](https://github.com/enginebai/MovieHunt) ⭐ 765 | 🐛 5 | 🌐 Kotlin | 📅 2022-07-29

  * Movie Android App written in Kotlin, MVVM, RxJava, Android Architecture Components.

* [SoundSpice-mobile](https://github.com/farshed/SoundSpice-mobile) ⭐ 427 | 🐛 23 | 🌐 JavaScript | 📅 2023-01-06

  * A light-weight and minimalist music player for Android

* [TvFlix](https://github.com/reactivedroid/TvFlix) ⭐ 411 | 🐛 0 | 🌐 Kotlin | 📅 2026-06-19

  * TvFlix android app using Dagger Hilt, Coroutines, Jetpack(Room, ViewModel, Paging, LiveData) based on MVVM architecture purely written in Kotlin.

* [MovieMade](https://github.com/michaelbel/Movies) ⭐ 338 | 🐛 0 | 🌐 Kotlin | 📅 2026-08-13

  * Moviemade - easy way to discover popular movies. This is a simple TMDb client for Android with material design.

* [Topcorn](https://github.com/theapache64/topcorn) ⭐ 228 | 🐛 4 | 🌐 Kotlin | 📅 2021-06-12

  * A minimalistic movie listing app to browse IMDB's top 250 movies, built to demonstrate MVVM with latest hot-trending Android development tools.

### :moneybag: Finance

* [Bitcoin-wallet](https://github.com/bitcoin-wallet/bitcoin-wallet) ⭐ 4,155 | 🐛 79 | 🌐 Java | 📅 2025-03-05

  * Bitcoin Wallet app for your Android device. Standalone Bitcoin node, no centralized backend required.

* [CoCoin](https://github.com/Nightonke/CoCoin) ⭐ 2,834 | 🐛 45 | 🌐 Java | 📅 2023-04-28

  * CoCoin, Multi-view Accounting Application

* [Gnucash-android](https://github.com/codinguser/gnucash-android) ⭐ 1,264 | 🐛 170 | 🌐 Java | 📅 2024-06-14

  * GnuCash Android is a companion expense-tracker application for GnuCash (desktop) designed for Android. It allows you to record transactions on-the-go and later import the data into GnuCash for the desktop.

* [Financius](https://github.com/mvarnagiris/financius) ⭐ 626 | 🐛 108 | 🌐 Java | 📅 2019-05-12

  * Financius is a simple application that helps you log and track your expenses and keep an eye on your balance.

* [bither-android](https://github.com/bither/bither-android) ⭐ 457 | 🐛 38 | 🌐 Java | 📅 2026-01-26

  * Bither is a simple and secure Bitcoin wallet!

* [Expenses](https://github.com/nominalista/expenses) ⭐ 404 | 🐛 16 | 🌐 Kotlin | 📅 2021-07-20

  * App written in Kotlin for budget tracking.

* [Finance-Manager](https://github.com/josharsh/Finance-Manager) ⭐ 23 | 🐛 5 | 🌐 Java | 📅 2023-09-26

  * Finance Manager is an Android Application which contains Income Tax Calculator and EMI Calculator.

* [Kripto](https://github.com/sauravrao637/Kripto) ⭐ 16 | 🐛 0 | 🌐 Kotlin | 📅 2022-04-21

  * Kripto is cryptocurrency and exchanges app for Android, written in Kotlin and made using Coingecko's API.

* [CurrencyExchange](https://github.com/Marwa-Eltayeb/CurrencyExchange) ⭐ 13 | 🐛 1 | 🌐 Kotlin | 📅 2022-04-12

  * Currency Exchange App displays real-time currency rates, shows a chart for any currency pair.

### :soccer: Games

* [Pixel-dungeon](https://github.com/watabou/pixel-dungeon) ⭐ 3,939 | 🐛 58 | 🌐 Java | 📅 2019-07-23

  * Traditional roguelike game with pixel-art graphics and simple interface

* [Santa-tracker-android](https://github.com/google/santa-tracker-android) ⚠️ Archived

  * Google Santa Tracker app for Android is an educational and entertaining tradition that brings joy to millions of children (and children at heart) across the world over the December holiday period.

* [Freeciv](https://github.com/freeciv/freeciv) ⭐ 1,577 | 🐛 8 | 🌐 C | 📅 2026-08-13

  * Freeciv is a Free and Open Source empire-building strategy game inspired by the history of human civilization.

* [2048-android](https://github.com/uberspot/2048-android) ⭐ 1,086 | 🐛 27 | 🌐 Java | 📅 2024-08-11

  * This is the Android port of the 2048 game made by Gabriele Cirulli.

* [CuboidalDrift](https://github.com/virejdasani/CuboidalDrift) ⭐ 32 | 🐛 3 | 🌐 C# | 📅 2023-03-20

  * Cuboidal Drift is a casual, fun android game with cool looking graphics, made by Virej Dasani.

* [QuizApp](https://github.com/pratyaksh1610/Quiz_App) ⭐ 25 | 🐛 10 | 🌐 Kotlin | 📅 2024-06-25

  * This is a Quiz App that Users can play to enjoy thier free time and also gain some knowledge on the go

### :weight\_lifting: Health & Fitness

* [Pedometer](https://github.com/j4velin/Pedometer) ⭐ 1,428 | 🐛 36 | 🌐 Java | 📅 2023-11-03

  * Lightweight pedometer app for Android using the hardware step sensor

* [Runnerup](https://github.com/jonasoreland/runnerup) ⭐ 947 | 🐛 217 | 🌐 Java | 📅 2026-08-12

  * Track your sport activities with RunnerUp using the GPS in your Android phone.

* [Android-heart-rate-monitor](https://github.com/phishman3579/android-heart-rate-monitor) ⭐ 474 | 🐛 14 | 🌐 Java | 📅 2020-10-01

  * Android based heart rate monitor which uses the camera and its flash to determine the users heart rate in beats per minute.

* [Petals](https://github.com/LeoColman/Petals) ⭐ 127 | 🐛 26 | 🌐 Kotlin | 📅 2026-08-11

  * The Open Source app Petals aims to help its users to either quit weed, reduce usage or simply know how much they're using.

* [Water-Intake-Tracker](https://github.com/JacobKerames/water-intake-tracker) ⭐ 2 | 🐛 0 | 🌐 Java | 📅 2023-05-17

  * Determine and track daily hydration goals based on body weight, and export data.

* [7minWorkout](https://github.com/sharshv2012/7minWorkout) ⭐ 0 | 🐛 0 | 🌐 Kotlin | 📅 2023-10-23

  * An workout app which tracks my workouts, has BMI calculator for different metrics and audio assistance.

* [FitLite](https://github.com/jay-sharmaa/FitLite) ⭐ 0 | 🐛 0 | 🌐 Kotlin | 📅 2025-10-04

  * A Home Fitness app which uses pose detection to detect the exercise performed and give voice automated voice feedback.

### :books: Libraries

* [RxJava](https://github.com/ReactiveX/RxJava) ⭐ 48,210 | 🐛 10 | 🌐 Java | 📅 2026-08-11

  * RxJava – Reactive Extensions for the JVM – a library for composing asynchronous and event-based programs using observable sequences for the Java VM.

* [Okhttp](https://github.com/square/okhttp/) ⭐ 47,043 | 🐛 135 | 🌐 Kotlin | 📅 2026-08-13

  * Square’s meticulous HTTP client for Java and Kotlin.

* [Retrofit](https://github.com/square/retrofit) ⭐ 43,927 | 🐛 172 | 🌐 Java | 📅 2026-08-11

  * A type-safe HTTP client for Android and the JVM

* [MPAndroidChart](https://github.com/PhilJay/MPAndroidChart) ⭐ 38,198 | 🐛 2,191 | 🌐 Java | 📅 2025-06-05

  * A powerful Android chart view / graph view library, supporting line- bar- pie- radar- bubble- and candlestick charts as well as scaling, panning and animations.

* [Lottie-android](https://github.com/airbnb/lottie-android) ⭐ 35,699 | 🐛 69 | 🌐 Java | 📅 2026-02-15

  * Render After Effects animations natively on Android and iOS, Web, and React Native

* [Glide](https://github.com/bumptech/glide) ⭐ 35,023 | 🐛 659 | 🌐 Java | 📅 2026-08-13

  * An image loading and caching library for Android focused on smooth scrolling

* [Zxing](https://github.com/zxing/zxing) ⭐ 34,068 | 🐛 0 | 🌐 Java | 📅 2026-07-28

  * ZXing ("Zebra Crossing") barcode scanning library for Java, Android

* [Butterknife](https://github.com/JakeWharton/butterknife) ⭐ 25,350 | 🐛 118 | 🌐 Java | 📅 2023-09-02

  * Bind Android views and callbacks to fields and methods.

* [EventBus](https://github.com/greenrobot/EventBus) ⭐ 24,719 | 🐛 147 | 🌐 Java | 📅 2024-02-21

  * Event bus for Android and Java that simplifies communication between Activities, Fragments, Threads, Services, etc. Less code, better quality.

* [Gson](https://github.com/google/gson) ⭐ 24,233 | 🐛 341 | 🌐 Java | 📅 2026-08-11

  * A Java serialization/deserialization library to convert Java Objects into JSON and back

* [ExoPlayer](https://github.com/google/ExoPlayer) ⭐ 21,929 | 🐛 627 | 🌐 Java | 📅 2025-12-23

  * ExoPlayer is an application level media player for Android. It provides an alternative to Android’s MediaPlayer API for playing audio and video both locally and over the Internet. ExoPlayer supports features not currently supported by Android’s MediaPlayer API, including DASH and SmoothStreaming adaptive playbacks.

* [Picasso](https://github.com/square/picasso) ⭐ 18,813 | 🐛 212 | 🌐 Kotlin | 📅 2024-11-06

  * A powerful image downloading and caching library for Android

* [Dagger](https://github.com/google/dagger) ⭐ 17,706 | 🐛 368 | 🌐 Java | 📅 2026-08-12

  * A fast dependency injector for Android and Java.

* [Logger](https://github.com/orhanobut/logger) ⭐ 13,844 | 🐛 81 | 🌐 Java | 📅 2022-09-14

  * Simple, pretty and powerful logger for android

* [Android-Debug-Database](https://github.com/amitshekhariitbhu/Android-Debug-Database) ⭐ 8,675 | 🐛 83 | 🌐 Java | 📅 2026-02-05

  * A library for debugging android databases and shared preferences - Make Debugging Great Again

* [Accompanist](https://github.com/chrisbanes/accompanist) ⭐ 7,829 | 🐛 11 | 🌐 Kotlin | 📅 2025-08-18

  * Accompanist is a group of libraries that contains some utilities which I've found myself copying around projects which use Jetpack Compose.

* [Junit5](https://github.com/junit-team/junit5/) ⭐ 7,042 | 🐛 114 | 🌐 Java | 📅 2026-08-13

  * This repository is the home of the next generation of JUnit, JUnit 5.

* [Toasty](https://github.com/GrenderG/Toasty) ⭐ 6,576 | 🐛 50 | 🌐 Java | 📅 2022-09-28

  * The usual Toast, but with steroids

* [PreviewSeekBar](https://github.com/rubensousa/PreviewSeekBar) ⭐ 3,460 | 🐛 10 | 🌐 Java | 📅 2024-01-10

  * A SeekBar suited for showing a preview of something. As seen in Google Play Movies.

* [MotionToast](https://github.com/Spikeysanju/MotionToast) ⭐ 1,533 | 🐛 32 | 🌐 Kotlin | 📅 2024-04-29

  * A Beautiful Multipurpose Motion Toast Library in Android using Kotlin heart\_eyes

* [AndroidCharts](https://github.com/HackPlan/AndroidCharts) ⭐ 1,323 | 🐛 23 | 🌐 Java | 📅 2021-05-12

  * An easy-to-use Android charts library with animation.

* [ZoomRecylerLayout](https://github.com/Spikeysanju/ZoomRecylerLayout) ⭐ 808 | 🐛 2 | 🌐 Kotlin | 📅 2020-07-22

  * An beautiful Zoom Animation Library for RecyclerView Items in Android using Kotlin.

* [ReactButton](https://github.com/amrdeveloper/reactbutton) ⭐ 111 | 🐛 3 | 🌐 Java | 📅 2023-05-25

  * Android Library to make it easy to add ReactButton feature in your app with Multi Reactions.

* [LottieDIalog](https://github.com/amrdeveloper/LottieDIalog) ⭐ 57 | 🐛 0 | 🌐 Java | 📅 2022-07-16

  * Android Library to create Lottie animation view dialog easily with a lot of customization.

* [FilterLibrary](https://github.com/hgayan7/FilterLibrary) ⭐ 44 | 🐛 0 | 🌐 Kotlin | 📅 2026-08-13

  * Android filter library -This library has 16 filters which can be applied to image bitmaps.

* [MathView](https://github.com/derysudrajat/math-view) ⭐ 31 | 🐛 3 | 🌐 Kotlin | 📅 2022-10-08

  * A Library for displaying math equations in Android

### :art: Misc

* [Jetsurvey](https://github.com/android/compose-samples/tree/master/Jetsurvey) ⭐ 23,375 | 🐛 7 | 🌐 Kotlin | 📅 2026-08-13

  * Jetsurvey is a sample survey app, built with Jetpack Compose. The goal of the sample is to showcase text input, validation and state capabilities of Compose.

* [Iosched](https://github.com/google/iosched) ⚠️ Archived

  * The Google I/O 2019 Android App

* [Plaid](https://github.com/nickbutcher/plaid) ⭐ 16,179 | 🐛 102 | 🌐 Kotlin | 📅 2024-01-04

  * An Android app which provides design news & inspiration as well as being an example of implementing material design.

* [UAMP](https://github.com/android/uamp) ⚠️ Archived

  * A sample audio app for Android

* [Pokedex](https://github.com/skydoves/Pokedex) ⭐ 8,350 | 🐛 26 | 🌐 Kotlin | 📅 2026-08-13

  * Android Pokedex using Dagger Hilt, Motion, Coroutines, Flow, Jetpack (Room, ViewModel, LiveData) based on MVVM architecture.

* [Duckduckgo](https://github.com/duckduckgo/android) ⭐ 4,782 | 🐛 172 | 🌐 Kotlin | 📅 2026-08-13

  * DuckDuckGo Android App

* [LeafPic](https://github.com/HoraApps/LeafPic) ⭐ 3,266 | 🐛 67 | 🌐 Java | 📅 2024-02-14

  * LeafPic is a fluid, material-designed alternative gallery, it also is ad-free and open source under GPLv3 license. It doesn't miss any of the main features of a stock gallery, and we also have plans to add more useful features.

* [Android-wikipedia](https://github.com/wikimedia/apps-android-wikipedia) ⭐ 3,000 | 🐛 33 | 🌐 Kotlin | 📅 2026-08-13

  * The official Wikipedia app for Android!

* [Ridesharing-uber-lyft-app](https://github.com/MindorksOpenSource/ridesharing-uber-lyft-app) ⭐ 1,672 | 🐛 4 | 🌐 Kotlin | 📅 2026-01-28

  * Ride-Sharing Uber Lyft Android App - Learn to build a ride-sharing Android Taxi Clone App like Uber, Lyft - Open-Source Project By MindOrks

* [Kickmaterial](https://github.com/byoutline/kickmaterial) ⭐ 1,612 | 🐛 2 | 🌐 Kotlin | 📅 2018-11-14

  * Crowdfunding app concept for Android. Created to showcase new trends in Android development with strong focus on Material Design.

* [AndroidTensorFlowMachineLearning](https://github.com/MindorksOpenSource/AndroidTensorFlowMachineLearningExample) ⭐ 1,440 | 🐛 13 | 🌐 Java | 📅 2025-05-02

  * Android TensorFlow MachineLearning Example (Building TensorFlow for Android)

* [ComposeAcademy-Playground](https://github.com/hitherejoe/ComposeAcademy-Playground) ⭐ 968 | 🐛 2 | 🌐 Kotlin | 📅 2021-04-08

  * Playground project for the Jetpack Compose APIs

* [Openfoodfacts-androidapp](https://github.com/openfoodfacts/openfoodfacts-androidapp) ⭐ 803 | 🐛 381 | 🌐 Kotlin | 📅 2026-08-10

  * Native version of Open Food Facts on Android - Coders & Decoders welcome

* [Compose-pokedex](https://github.com/zsoltk/compose-pokedex) ⭐ 797 | 🐛 2 | 🌐 Kotlin | 📅 2022-05-10

  * Pokedex on Jetpack Compose

* [CrashReporter](https://github.com/MindorksOpenSource/CrashReporter) ⭐ 403 | 🐛 12 | 🌐 Java | 📅 2023-12-30

  * CrashReporter is a handy tool to capture app crashes and save them in a file.

* [MoonShot](https://github.com/haroldadmin/MoonShot) ⭐ 163 | 🐛 7 | 🌐 Kotlin | 📅 2020-10-02

  * MoonShot is a SpaceX companion app to help you keep up with their launches. It uses the SpaceX API to fetch data.

* [Space-app](https://github.com/ValterKasper/space-app) ⭐ 149 | 🐛 1 | 🌐 Kotlin | 📅 2022-07-23

  * An Android app which shows timeline of upcoming rocket launches and showcases architecture of real application.

* [Android-conferences](https://github.com/jitinsharma/android-conferences) ⭐ 26 | 🐛 1 | 🌐 Kotlin | 📅 2022-06-02

  * An app which displays conferences from <http://androidstudygroup.github.io/conferences/>

* [CoronavirusWorldStatus](https://github.com/HamdiBoumaiza/CoronavirusWorldStatus) ⭐ 21 | 🐛 1 | 🌐 Kotlin | 📅 2020-09-05

  * An app to stay up to date with the latest stats of the coronavirus , using Kotlin with MVVM ,Coroutines , Android Architecture Components and Dagger

* [Zap.Store](https://github.com/zapstore)

  * A permissionless app store that allows users to verify apps, devs to sign their own apps, and users to discover new & exciting apps via their social graph! Zap Store makes it easy for enthusiasts & users to reward their app's dev via micropayments, and enables the dev to build a commuity around their app.

### :newspaper: News

* [JetNews](https://github.com/android/compose-samples/tree/master/JetNews) ⭐ 23,375 | 🐛 7 | 🌐 Kotlin | 📅 2026-08-13

  * Jetnews is a sample news reading app, built with Jetpack Compose. The goal of the sample is to showcase the current UI capabilities of Compose.

* [Materialistic](https://github.com/hidroh/materialistic) ⭐ 2,334 | 🐛 432 | 🌐 Java | 📅 2024-05-08

  * A material-design Hacker News Android reader

* [Flym](https://github.com/FredJul/Flym) ⚠️ Archived

  * Flym News Reader is a light Android feed reader (RSS/Atom)

* [Nextcloud News Reader](https://github.com/nextcloud/news-android) ⭐ 768 | 🐛 116 | 🌐 Java | 📅 2026-08-13

  * Android client for the Nextcloud news/feed reader app.

* [NYTimes-App](https://github.com/TheCodeMonks/NYTimes-App) ⭐ 513 | 🐛 4 | 🌐 Kotlin | 📅 2022-03-13

  * A Simple Demonstration of the New York Times App using Jsoup web crawler with MVVM Architecture

* [Cheddar](https://github.com/adrianblancode/Cheddar/) ⭐ 118 | 🐛 2 | 🌐 Kotlin | 📅 2026-01-11

  * Hacker News reader for Android, written in Compose

* [Newzz-Compose](https://github.com/Akashkamble/Newzz-Compose) ⭐ 33 | 🐛 0 | 🌐 Kotlin | 📅 2022-03-18

  * News App built with Jetpack Compose

* [NewsFeed](https://github.com/KevinGitonga/NewsFeed) ⭐ 15 | 🐛 0 | 🌐 Kotlin | 📅 2020-05-13

  * A localized News reader Android app powered by newsapi.org

* [NewsFeed-MVI-Dagger](https://github.com/HadySalhab/NewsFeed-MVI-Dagger) ⭐ 14 | 🐛 1 | 🌐 Kotlin | 📅 2020-03-26

  * Android News Application built in kotlin: MVI+Dagger+NetworkBoundResource+ViewModel+Livedata.

* [Awesome NewsApp](https://github.com/Mehranalam/NewsApp) ⭐ 4 | 🐛 0 | 🌐 Java | 📅 2022-07-24

  * A Awesome news app using volley library and picasso library This app uses an open source API for its news sources.

### :rocket: Productivity

* [Joplin](https://github.com/laurent22/joplin) ⭐ 55,934 | 🐛 650 | 🌐 TypeScript | 📅 2026-08-12

  * Joplin - an open source note taking and to-do application with synchronization capabilities for Windows, macOS, Linux, Android and iOS.

* [uhabits](https://github.com/iSoron/uhabits) ⭐ 10,127 | 🐛 48 | 🌐 Kotlin | 📅 2026-07-21

  * It is an mobile app that helps you create and maintain good habits, allowing you to achieve your long-term goals. Detailed graphs and statistics show you how your habits improved over time. It is completely ad-free and open source.

* [AmazeFileManager](https://github.com/TeamAmaze/AmazeFileManager) ⭐ 6,334 | 🐛 636 | 🌐 Kotlin | 📅 2026-08-13

  * Material design file manager for Android

* [Owncloud - Android](https://github.com/owncloud/android) ⭐ 4,152 | 🐛 199 | 🌐 Kotlin | 📅 2026-08-13

  * The ownCloud Android App

* [Omni-Notes](https://github.com/federicoiosue/Omni-Notes) ⭐ 2,804 | 🐛 213 | 🌐 Java | 📅 2025-08-12

  * Open source note-taking application for Android

* [Etar](https://github.com/Etar-Group/Etar-Calendar/) ⭐ 2,574 | 🐛 395 | 🌐 Java | 📅 2026-08-02

  * An enhanced fork of the AOSP calendar app

* [Minimal-Todo](https://github.com/avjinder/Minimal-Todo) ⭐ 2,190 | 🐛 98 | 🌐 Java | 📅 2023-05-22

  * Material To-Do App

* [AnExplorer](https://github.com/1hakr/AnExplorer) ⭐ 2,012 | 🐛 36 | 🌐 Java | 📅 2026-06-02

  * Another Android Explorer ( File Manager ) is an All-in-One Open source file manager. AnExplorer File Manager (File Explorer) is designed for all android devices including Phones, Phablets, Tablets, Chromecast, Wear OS, Android TV and Chromebooks. It's a fully designed with Material guidelines by Google.

* [MLManager](https://github.com/javiersantos/MLManager) ⭐ 1,249 | 🐛 6 | 🌐 Java | 📅 2020-09-30

  * A modern, easy and customizable app manager for Android with Material Design

* [Clean-Notes](https://github.com/mitchtabian/Clean-Notes) ⭐ 556 | 🐛 3 | 🌐 Kotlin | 📅 2024-03-13

  * Clean Architecture by layer

* [Scarlet-Notes](https://github.com/BijoySingh/Scarlet-Notes) ⭐ 361 | 🐛 66 | 🌐 Kotlin | 📅 2021-12-21

  * Simple yet powerful rich note taking android application, with a lot of flexibilty of usage.

* [Linkhub](https://github.com/amrdeveloper/linkhub) ⭐ 328 | 🐛 37 | 🌐 Kotlin | 📅 2026-08-09

  * Open source link manager with folders and widget feature and sort links depend on usage.

* [JetpackComposeCalculator](https://github.com/ahmedrizwan/JetpackComposeCalculator) ⭐ 314 | 🐛 2 | 🌐 Kotlin | 📅 2020-08-11

  * A jetpack compose clone of Android 10 Calculator UI

* [HabitBuilder](https://github.com/ofalvai/HabitBuilder) ⭐ 290 | 🐛 40 | 🌐 Kotlin | 📅 2026-08-03

  * A simple and clean app with the right tools to keep track of habits and understand progress.

* [LeetDroid](https://github.com/cdhiraj40/LeetDroid) ⚠️ Archived

  * LeetDroid is an android client for LeetCode.
  *

* [Compose-ShoppingList](https://github.com/QArtur99/Compose-ShoppingList) ⭐ 65 | 🐛 0 | 🌐 Kotlin | 📅 2023-11-23

  * Jetpack compose shopping list app

* [Studify](https://github.com/cmcodes1/Studify) ⭐ 20 | 🐛 11 | 🌐 JavaScript | 📅 2023-01-06

  * ☑️ Student Utility Mobile App. Features: Mark Attendance, View Time Table, Create Tasks.

* [StackOverflow - Community Version](https://github.com/mayorJAY/StackOverflow-Community-Version) ⭐ 17 | 🐛 0 | 🌐 Kotlin | 📅 2024-11-03

  * This is an Android StackOverflow client application implemented using the MVVM architecture, Retrofit, LiveData, Flow, ViewModel, Repository pattern, View Binding. Users can get to view Questions which have been asked on Stack Overflow; picking a particular Question makes the user view it in detail as well as the Answers provided. Users can also search for a particular problem they are having by typing in a search query or by capturing an image and performing an OCR on it.

* [Work Tracker](https://github.com/parmstrong842/WorkTracker) ⭐ 17 | 🐛 4 | 🌐 Kotlin | 📅 2024-10-29

  * Worktracker is a comprehensive and user-friendly app designed to help you keep a record of your work hours. With its intuitive interface, you can effortlessly log in and out of your shift, no matter where you are. Whether you're clocking in at the office or logging your hours from home, the app has got you covered.

* [OSSAVE](https://github.com/mukuldeep/OSSAVE) ⭐ 8 | 🐛 17 | 🌐 Java | 📅 2025-07-12

  * Open Source Simple Android Video Editor

* [Notes-Central](https://github.com/vidit135g/Notes-Central) ⭐ 7 | 🐛 1 | 🌐 Java | 📅 2020-07-10

  * Simple note taking app.

### :hammer\_and\_wrench: Tools

* [SoundRecorder](https://github.com/dkim0419/SoundRecorder) ⭐ 1,500 | 🐛 77 | 🌐 Java | 📅 2026-06-23

  * A simple sound recording app implementing Material Design

* [KDEConnect](https://github.com/KDE/kdeconnect-android) ⭐ 1,413 | 🐛 0 | 🌐 Kotlin | 📅 2026-08-13

  * KDE Connect is a multi-platform app that allows your devices to communicate (eg: your phone and your computer).

* [Turbo-editor](https://github.com/vmihalachi/turbo-editor) ⭐ 1,365 | 🐛 77 | 🌐 Java | 📅 2020-03-03

  * Simple and powerful File Editor for Android. All is licensed under the GPLv3 license.

* [Wally](https://github.com/Musenkishi/wally) ⭐ 956 | 🐛 10 | 🌐 Java | 📅 2019-06-07

  * Wally is a fast and efficient wallpaper application for Android

* [Red-moon](https://github.com/LibreShift/red-moon) ⭐ 727 | 🐛 65 | 🌐 Kotlin | 📅 2026-05-18

  * Android screen filter app for night time phone use.

* [Google Science Journal](https://github.com/google/science-journal) ⚠️ Archived

  * Use the sensors in your mobile devices to perform science experiments.

* [Hash Checker](https://github.com/hash-checker/hash-checker) ⚠️ Archived

  * Fast and simple application for generating and comparison hashes from files or text

* [GitFox](https://gitlab.com/terrakok/gitlab-client)

  * Client to manage GitLab projects. Clean Architecture implementation

### :earth\_africa: Travel

* [Travel-Mate](https://github.com/Swati4star/Travel-Mate) ⭐ 1,337 | 🐛 37 | 🌐 Java | 📅 2022-06-13

  * Travel Mate is an android app for travellers. It provides features like choosing the correct destination, making bookings, and organizing the trip. It provides solutions for every problem a traveller might face during their journey. It provides a comprehensive list of information about a destination: current weather, best places to hangout, and city's current trends.

* [Transportr](https://github.com/grote/Transportr) ⭐ 1,176 | 🐛 115 | 🌐 Kotlin | 📅 2026-05-28

  * The public transport companion that respects your privacy and your freedom. Transportr is a non-profit app developed by people around the world to make using public transport as easy as possible wherever you are.

* [Cyclestreets](https://github.com/cyclestreets/android) ⭐ 225 | 🐛 104 | 🌐 Java | 📅 2025-06-11

  * CycleStreets is a UK-wide cycle journey planner system, which lets you plan routes from A to B to C by bike. It is designed by cyclists, for cyclists, and caters to the needs of both confident and less confident cyclists.

* [HolidayInfo](https://github.com/mayorJAY/HolidayInfo) ⭐ 6 | 🐛 0 | 🌐 Kotlin | 📅 2025-04-08

  * HolidayInfo is a multi-module, offline-first Android Application that fetches and displays the various holidays observed in different Countries and different years. The App fetches the data from Nager.Date API and uses Room for offline caching. It is implemented using Multi-module, Clean Architecture, Model-View-ViewModel (MVVM) pattern and uses Modern Android Development pattern and libraries.

### :gear: Utilities

* [Bitwarden](https://github.com/bitwarden/android) ⭐ 9,216 | 🐛 174 | 🌐 Kotlin | 📅 2026-08-13

  * Password management solution for individuals, teams, and business organizations written in C# with Xamarin Android, Xamarin iOS, and Xamarin Forms.

* [Simple-Calendar](https://github.com/SimpleMobileTools/Simple-Calendar) ⭐ 3,647 | 🐛 306 | 🌐 Kotlin | 📅 2024-06-26

  * A simple calendar with events, customizable widgets and no ads.

* [Authpass](https://github.com/authpass/authpass) ⭐ 2,763 | 🐛 166 | 🌐 Dart | 📅 2026-08-13

  * AuthPass - Password Manager based on Flutter for all platforms. Keepass 2.x (kdbx 3.x) compatible.

* [Images-to-PDF](https://github.com/Swati4star/Images-to-PDF) ⭐ 1,327 | 🐛 97 | 🌐 Java | 📅 2026-01-08

  * An app to convert images to PDF file!

* [AnotherMonitor](https://github.com/AntonioRedondo/AnotherMonitor) ⭐ 1,112 | 🐛 10 | 🌐 Java | 📅 2023-01-23

  * Monitors and records the CPU and memory usage of Android devices

* [Photo-affix](https://github.com/afollestad/photo-affix) ⚠️ Archived

  * Stitch your photos together vertically or horizontally easily!

* [KeyPass](https://github.com/yogeshpaliyal/KeyPass) ⭐ 782 | 🐛 31 | 🌐 Kotlin | 📅 2026-07-18

  * Offline Password manager for android, build in compose and material you, support backup encrypted database, etc.

* [Pdfview-Android](https://github.com/Dmitry-Borodin/pdfview-android) ⭐ 306 | 🐛 27 | 🌐 Java | 📅 2022-06-13

  * Small Android library to show PDF files

* [Talalarmo](https://github.com/trikita/talalarmo) ⭐ 237 | 🐛 22 | 🌐 Java | 📅 2020-05-25

  * Minimal, simple and beautiful alarm clock thoughtfully designed by nap enthusiasts.

* [Memento-Calendar](https://github.com/alexstyl/Memento-Calendar) ⚠️ Archived

  * Memento Calendar is a modern namedays app for Android. This repository contains the source code of Memento Calendar.

* [Earthview-android](https://github.com/PDDStudio/earthview-android) ⭐ 135 | 🐛 5 | 🌐 Java | 📅 2017-05-11

  * A library to include 'EarthView with Google' into your application with ease.

* [WiFiKeyShare](https://github.com/bparmentier/WiFiKeyShare) ⚠️ Archived

  * Share Wi-Fi passwords with QR codes or NFC tags

* [Moticons](https://github.com/javiersantos/Moticons) ⭐ 76 | 🐛 0 | 🌐 Java | 📅 2023-12-01

  * A simple, easy and modern app to manage and use Japanese Emoticons with Material Design

* [Varis-Android](https://github.com/dkhmelenko/Varis-Android) ⭐ 45 | 🐛 16 | 🌐 Java | 📅 2021-07-29

  * Android client for Travis CI

* [ImageSearching](https://github.com/MinhyeokOh/android_app_ui) ⭐ 4 | 🐛 0 | 🌐 Java | 📅 2023-06-05
  * Quickly find photos stored in the Android gallery using tags, date taken, and location, select one photo to write a diary, and select keyword from the diary to create additional tags

### :partly\_sunny: Weather

* [Good-weather](https://github.com/qqq3/good-weather) ⭐ 250 | 🐛 49 | 🌐 Java | 📅 2026-03-16

  * Open source weather app for Andorid

* [CoolWeather](https://github.com/akoufa/CoolWeather) ⭐ 198 | 🐛 3 | 🌐 Kotlin | 📅 2023-04-22

  * Weather App that uses Android best practices. Android Jetpack, clean architecture. Written in Kotlin

* [WeatherForecast](https://github.com/Marwa-Eltayeb/WeatherForecast) ⭐ 24 | 🐛 0 | 🌐 Kotlin | 📅 2022-04-16

  * Weather Forecast App implemented in MVP Architecture using Kotlin that enable users to have instant access to weather data.

* [WeatherZone](https://github.com/SoumikBhatt/WeatherZone) ⭐ 10 | 🐛 2 | 🌐 Kotlin | 📅 2023-07-13

  * Simple Weather app for Android following MVVM architecture, and Jetpack Component like Room etc.

* [Weathercast](https://github.com/vidit135g/Weathercast) ⭐ 4 | 🐛 1 | 🌐 Java | 📅 2020-09-17

  * Minimal weather app for Android

* [Sunshine](https://github.com/pragyaSingh13/Sunshine) ⭐ 1 | 🐛 1 | 🌐 Java | 📅 2021-10-08

  * Weather app to get current weather info of any city around the globe using OpenWeatherMap API.

## :dart:  **HacktoberFest**

<p align="center">
<img width="720px" src="https://raw.githubusercontent.com/binaryshrey/Awesome-Android-Open-Source-Projects/refs/heads/master/assets/Hacktoberfest%202025.png" alt="hacktoberfest"></img>
</p>

[Hacktoberfest](https://hacktoberfest.digitalocean.com/) is a month-long celebration of open source software organized by DigitalOcean in partnership with [GitHub](https://github.com/) and [Dev](https://dev.to/). Hacktoberfest is open to everyone in our global community. Whether you’re new to development, a student, long-time contributor, event host, or company of any size, you can help drive growth of open source and make positive contributions to an ever-growing community. All backgrounds and skills levels are encouraged to complete the challenge.

To qualify for the official Hacktoberfest swag options, you must [register](https://hacktoberfest.digitalocean.com/) and make four pull requests between October 1-31. Pull requests can be made to any public repository on GitHub, not just the ones with issues labeled Hacktoberfest. If a maintainer reports your pull request as spam or behavior not in line with [Hacktoberfest’s quality standards](https://hacktoberfest.digitalocean.com/details#quality), you will be ineligible to participate.

You can also check the [swag-list](https://hacktoberfestswaglist.com/) and other companies giving away swags for HacktoberFest.

## :memo:  **Contributing**

See [CONTRIBUTING.md](https://github.com/binaryshrey/Awesome-Android-Open-Source-Projects/blob/master/CONTRIBUTING.md) ⭐ 1,238 | 🐛 8 | 📅 2026-03-26

***

> _Enhansomed by [enhansome](https://github.com/enhansome) on 2026-08-13._
